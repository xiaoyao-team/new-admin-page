<template>
  <div>
    ...
    <div class="activity-group-card">
      <el-card>
        <div slot="header">
          <span style="line-height: 30px;">Ios引导页列表</span>
        </div>
        <div class="activity-group-panel">
          <div class="group-item" @click="goAdd">
            ...
            <el-button type="text" style="font-size:16px">添加Ios引导页</el-button>
...
export default {
  ...
  computed: {
    ...
    apkPageData() {
      if (addIosModule.apkPageData) {
        return addIosModule.apkPageData.filter(data => (this.nowgame!=-1)?data.appId==this.nowgame:data);
      }else{
        return []
      }
    }
  ...
  methods: {
    goAdd(){
      this.$emit('handleChange', 'AddIosPage');
    },
    goEdit(data){
      addIosModule.SET_DEFAULTDATA( data )
      this.$emit('handleChange', 'editIosPage');
    },
    delApkPage(data){
      handleConfirm("是否确定删除" + data.pageTitle + "活动组 ?", "warning")
        .then(() => {
          addIosModule.delApkPage({ id: data.id, filePath: data.filePath }).then(() => {
            this.$message({
